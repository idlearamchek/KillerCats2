...
- (BOOL) inputEnteredWithWorlPos:(CGPoint)worldLocation;
- (void) inputUpInside;
- (void) inputUpOutside;
...
- (void) touchEntered:(CCTouch*)touch withEvent:(CCTouchEvent*)event
{
    ...
    BOOL wasHandled = [self inputEnteredWithWorlPos:worldLocation];
    
    //XZ hack to change pos on touchBegan
    if(!wasHandled && [_background hitTestWithWorldPos:worldLocation]){
        CGPoint dragPos = [touch locationInNode:self];
        
        _draggingHandle = YES;
        self.highlighted = YES;
        _handleStartPos = _handle.position;
        _dragStartPos = _handle.position;
...
- (void) layout
{
    ...
    self.hitAreaExpansion = CGSizeMake(_handle.contentSizeInPoints.width / 2.0f, _handle.contentSizeInPoints.width / 2.0f);
